...
bool OpenCLProgram::addKernel(const io::stringc &EntryPoint)
{
    ...
    if (OpenCLDevice::checkForError(Error, "Creating OpenCL kernel \"" + EntryPoint + "\" failed"))
        return false;
    ...
    const io::stringc &EntryPoint, s32 Dimensions, const u32* GlobalWorkSizes, const u32* LocalWorkSizes)
{
    ...
    return !OpenCLDevice::checkForError(Error, "Executing OpenCL program failed");
}
...
bool OpenCLProgram::setParameter(const io::stringc &EntryPoint, u32 Index, const void* Buffer, u32 Size)
{
    ...
    return !OpenCLDevice::checkForError(Error, "Setting OpenCL kernel parameter (" + io::stringc(Index) + ") failed");
}
...
bool OpenCLProgram::buildProgram(const io::stringc &SourceString, const io::stringc &CompilationOptions)
{
    ...
    if (OpenCLDevice::checkForError(Error, "Creating OpenCL program failed"))
        return false;
    ...
    return !OpenCLDevice::checkForError(Error, "Building OpenCL program failed");
}
